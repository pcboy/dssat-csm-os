...
!    INTEGER :: FNUMWRK                 ! File number,work file          #          ! (From RunInit)   
    ...
    CHARACTER (LEN=255) CFGDFILE      ! Configuration directory+file              ! (From RunInit)  
    ...
    CHARACTER (LEN=255)  FAPPLINE(30)  ! Fertilizer application details            ! (From SeasInit) 
    CHARACTER (LEN=255) FILEA         ! Name of A-file                            ! (From SeasInit) 
    CHARACTER (LEN=107) FILEADIR      ! Name of A-file directory                  ! (From SeasInit) 
    CHARACTER (LEN=255) FILEIO        ! Name of input file,after check            ! (From RunInit)  
    CHARACTER (LEN=3)   FILEIOT       ! Type of input file                        ! (From RunInit)  
    CHARACTER (LEN=255) FILENEW       ! Temporary name of file                    ! (From SeasInit) 
    CHARACTER (LEN=255) FILET         ! Name of T-file                            ! (From Output)   
    CHARACTER (LEN=255) FILEX         ! Name of X-file                            ! (From SeasInit) 
    CHARACTER (LEN=1)   FNAME         ! File name switch (N->standard)            ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEERA      ! File name,A-errors                        ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEERT      ! File name,T-errors                        ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEEVAL     ! File name,evaluate outputs                ! (From RunInit)  
    CHARACTER (LEN=255) FNAMELEAVES   ! File name,leaves outputs                  ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEMEAS     ! File name,measured outputs                ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEOV       ! File name,overview outputs                ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPHASES   ! File name,phases outputs                  ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPHENOLM  ! File name,phenology measured              ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPHENOLS  ! File name,phenology outputs               ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPREM     ! File name,responses,measured              ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPRES     ! File name,responses,simulated             ! (From RunInit)  
    CHARACTER (LEN=255) FNAMEPSUM     ! File name,plant summary                   ! (From RunInit)  
    ...
    CHARACTER (LEN=255)  LINESTAR      ! Group header line (with star)             ! (From Output)   
    CHARACTER (LEN=255)  LINESTAR2     ! Group header line (with star)             ! (From Output)   
    ...
    CHARACTER (LEN=255)  PATHCR        ! Path to genotype (CUL) files              ! (From SeasInit) 
    CHARACTER (LEN=255)  PATHEC        ! Path to genotype (ECO) files              ! (From SeasInit) 
    CHARACTER (LEN=255)  PATHSP        ! Path to genotype (SPE) files              ! (From SeasInit) 
